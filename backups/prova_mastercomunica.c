...
/*
	Qui il master calcola la porzione di byte che ogni processo deve elaborare
	Es: il processo 3 lavora dal byte 400 del file 5 fino al byte 150 del file 7
	Dopodiché vengono inviati i dati agli slave ed ogni processo chiama il metodo wordCount
*/
Word* chunkAndCount(int myrank, int totalByte, fileStats* myFiles, int countFiles, int p, int *size){
	int resto = totalByte % p;
	int byteProcess = totalByte / p;
	int inizioFile = 0, fineFile = 0, inizio = 0, fine = 0;											//da che file parto a che file arrivo, da che byte parto a che byte arrivo
	int sendBuf[p][4], recvBuf[4];
	if(myrank==0){																			
		for(int processo = 0; processo<p; processo++){																	
			sendBuf[processo][0] = (processo<resto) ? (byteProcess + 1) * processo : resto + (byteProcess * processo);				//da che byte parto (riferito ai byte totali da elaborare)
			sendBuf[processo][1] = (processo<resto) ? (byteProcess + 1) * (processo + 1) : resto + (byteProcess * (processo + 1));	//a che byte arrivo (riferito ai byte totali da elaborare)
			...
			for(int i = 0; i<countFiles; i++){
				if(sendBuf[processo][0]<=myFiles[i].size){
					sendBuf[processo][2] = i;
					break;
				}else
					sendBuf[processo][0] -= myFiles[i].size;																		//da che byte parto (riferito al file, es: byte 400 del file 5)
			}
			for(int i = 0; i<countFiles; i++){
				if(sendBuf[processo][1]<=myFiles[i].size){
					sendBuf[processo][3] = i;
					break;
				}else
					sendBuf[processo][1] -= myFiles[i].size;																		//a che byte arrivo (riferito al file, es: byte 150 del file 7)
			}
	...
	inizio = recvBuf[0];	fine = recvBuf[1];	inizioFile = recvBuf[2];	fineFile = recvBuf[3];
	//printf("Processore: %d, File inizio: %d, Byte: %d - File fine: %d, Byte: %d\n",myrank,inizioFile,inizio,fineFile,fine);
